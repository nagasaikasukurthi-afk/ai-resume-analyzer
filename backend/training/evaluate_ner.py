...
TEST_FILE = Path("data/processed/ner/test.json")
...
print(f"Test resumes: {len(test_data)}")
...
for index, example in enumerate(test_data):
    ...
    for token_index, word_id in enumerate(word_ids):
        ...
        predicted_labels.append(
            model.config.id2label[int(predictions[token_index])]
        )
...
for index in range(min(3, len(test_data))):
